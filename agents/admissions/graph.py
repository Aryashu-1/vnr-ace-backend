...
from agents.admissions.nodes import (
    public_supervisor_agent, faq_agent, 
    department_router_agent, create_department_agent
)
...
_admissions_graph = None

def get_admissions_graph():
    global _admissions_graph
    if _admissions_graph is None:
        _admissions_graph = compile_admissions_graph()
    return _admissions_graph

def invalidate_admissions_graph():
    global _admissions_graph
    _admissions_graph = None

# For backward compatibility with existing imports
class AdmissionsGraphProxy:
    def ainvoke(self, *args, **kwargs):
        return get_admissions_graph().ainvoke(*args, **kwargs)
    
    def invoke(self, *args, **kwargs):
        return get_admissions_graph().invoke(*args, **kwargs)

    def stream(self, *args, **kwargs):
        return get_admissions_graph().stream(*args, **kwargs)

    def astream(self, *args, **kwargs):
        return get_admissions_graph().astream(*args, **kwargs)

admissions_graph = AdmissionsGraphProxy()
